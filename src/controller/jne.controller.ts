...
import RequestTariffDto from "../services/jne/dto/request-tariff.dto";
import RequestOrderDto from "../services/jne/dto/request-generate.dto";
...
export class JneController {
  static checkTariff = async (
    ...
    if (error) {
      ...
      try {
        ...
        const tariff = await jneService.checkTariff(dataGetTarif);
  ...
  static createOrder = async (
    ...
    if (error) {
      ...
      try {
        ...
        const jneService = new JneService();
        const order = await jneService.createOrder(payload);
  ...
  static cancelOrder = async (
    ...
    try {
      ...
      const result = await jneService.cancelOrder(order_id);
  ...
  static trackStatus = async(
    ...
    try {
      const jneService = new JneService();
      const { awb } = req.body;
      const trackData = await jneService.getTrackingStatus(awb);
      return res.status(200).json({
        status: true,
        data: trackData       
      })
